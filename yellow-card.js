...
const PLAYER_STORAGE_KEY = 'yellowCardCurrentPlayer';
...
function setGameVisible(flag) {
  [currentRoomBox, playersPanel, topicsPanel, wordsPanel].forEach(el => el.classList.toggle('hidden', !flag));
  if (!flag) {
...
function renderRoomCards(roomDocs) {
  ...
  roomCardsEl.innerHTML = ROOM_CONFIGS.map(config => {
    ...
    const count = data.playerCount ?? (Array.isArray(data.playerIds) ? data.playerIds.length : 0);
    ...
    const buttonDisabled = full || (disabled && !isCurrent) ? 'disabled' : '';
    return `
      <div class="room-card ${isCurrent ? 'active' : ''}">
        ...
        <button data-room="${config.id}" ${buttonDisabled}>${buttonLabel}</button>
      </div>`;
  }).join('');
}
...
async function joinRoom(roomId) {
  ...
  try {
    await runTransaction(db, async transaction => {
      ...
      transaction.update(roomRef, { playerCount: count + 1, playerIds: ids });
    });
...
function renderPlayers() {
  ...
  playerTableBody.innerHTML = state.players.map(player => {
    ...
    return `
      <tr class="${isCurrent ? 'current-player' : ''} ${eliminated ? 'player-eliminated' : ''}">
...
async function adjustYellow(playerId, delta) {
  ...
  try {
    await runTransaction(db, async transaction => {
      ...
      let yellow = (snap.data().yellow ?? 0) + delta;
      yellow = Math.max(0, Math.min(3, yellow));
...
async function dealCards() {
  ...
  try {
    await runTransaction(db, async transaction => {
      ...
      const ids = Array.isArray(data.playerIds) ? data.playerIds : state.players.map(p => p.id);
      if (deck.length < ids.length * 13) deck = buildWordDeck();
...
async function resetRoomState() {
  if (!state.currentRoomId) return;
  if (!confirm('將重洗題目、重洗詞語卡並清除所有黃牌與手牌，確定要重新開始？')) return;
  try {
    const roomRef = doc(db, 'yellowRooms', state.currentRoomId);
    await updateDoc(roomRef, {
      topicDeck: buildTopicDeck(),
...
playerTableBody.addEventListener('click', event => {
  ...
  if (action === 'draw-word') drawWordForPlayer(playerId);
});
